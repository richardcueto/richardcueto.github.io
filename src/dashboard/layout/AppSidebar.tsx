...
// Importaciones exactas de tu archivo icons
import {
  BoxCubeIcon,
  CalenderIcon,
  ChevronDownIcon,
...
// Componente helper para evitar que un SVG 'undefined' rompa la pantalla
const SafeIcon = ({ icon }: { icon: React.ReactNode }) => {
  if (!icon) return <span className="w-5 h-5 inline-block bg-gray-600 rounded-full" />;
  return <>{icon}</>;
};
            ...
            <div>
              ...
              >
                {isExpanded || isHovered || isMobileOpen ? (
                  ...
                  <SafeIcon icon={<HorizontaLDots className="size-6" />} />
                )}
              </h2>

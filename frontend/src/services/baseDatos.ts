...
export async function supabase_insert(title:string,paragraph:string,content:string,image:string,tags:any,publishDate:string,authorId:string|number):Promise<void> {
  ...
  alert('¡Artículo guardado exitosamente!');
        ...
        // CRUD PERFECTO
interface supabaseSelectParametros{
  tabla:string;
  select?:string;
  where?:Record<string,any>;
}
export async function supabase_selecionar({tabla, select ='*', where={}}:supabaseSelectParametros) {

  let query = supabase.from(tabla).select(select);

  Object.entries(where).forEach(([columna, valor]) => {
...
interface supabaseInsertarParametros{
  tabla:string;
  insert: Record<string, any>;
}
export async function supabase_insertar({tabla, insert}:supabaseInsertarParametros) {

  let query = supabase.from(tabla).insert(insert);
...
interface supabaseActualizarParametros{
  tabla:string;
  update:Record<string,any>;
  where?:Record<string,any>;
}
export async function supabase_Actualizar({tabla, update, where={}}:supabaseActualizarParametros) {

  let query = supabase.from(tabla).update(update);
...
interface supabaseEliminarParametros{
  tabla:string;
  where?:Record<string,any>;
}
export async function supabase_Eliminar({tabla, where={}}:supabaseEliminarParametros) {
